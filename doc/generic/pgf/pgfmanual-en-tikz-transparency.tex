...
\section[tikz-transparency]{Transparency}
...
\end{codeexample}
    ...
    \def\showmodeR#1{\leavevmode
        \begin{minipage}[t]{88.5mm}\raggedright#1\end{minipage} \\
    }
    % \def\makeline#1#2#3{
    %   #1 & #2 & \begin{minipage}[t]{88.5mm}\raggedright#3\end{minipage}
    %   \textcolor{black!25}{\hrule height1pt} \\
    % }
    % \newcommand{\makeline}[3]{
    %   #1 & #2 & #3 \\
    % }
    \def\showmode#1{
    ...
    \showmode{normal} & \verb|normal| & \showmodeR{When painting a pixel with a some color (called the
        ...
        completely ignored.}
    \showmode{multiply} & \verb|multiply| & \showmodeR{Multiplies the backdrop and source color values. The
    ...
    \showmode{screen} & \verb|screen| & \showmodeR{Multiplies the complements of the backdrop and source
    ...
    \showmode{overlay} & \verb|overlay| & \showmodeR{Multiplies or screens the colors, depending on the
    ...
    \showmode{darken} & \verb|darken| & \showmodeR{Selects the darker of the backdrop and source colors. The
        backdrop is replaced with the source where the source is darker;
        otherwise, it is left unchanged.}
    \showmode{lighten} & \verb|lighten| & \showmodeR{Selects the lighter of the backdrop and source colors.
        The backdrop is replaced with the source where the source is lighter;
        otherwise, it is left unchanged.}
    \showmode{color dodge} & \verb|color dodge| & \showmodeR{Brightens the backdrop color to reflect the source
        color. Painting with black produces no changes.}
    \showmode{color burn} & \verb|color burn| & \showmodeR{Darkens the backdrop color to reflect the source
        color. Painting with white produces no change.}
    \showmode{hard light} & \verb|hard light| & \showmodeR{Multiplies or screens the colors, depending on the
        source color value. The effect is similar to shining a harsh spotlight
        on the backdrop.}
    \showmode{soft light} & \verb|soft light| & \showmodeR{Darkens or lightens the colors, depending on the
        source color value. The effect is similar to shining a diffused
        spotlight on the backdrop.}
    \showmode{difference} & \verb|difference| & \showmodeR{Subtracts the darker of the two constituent colors
        from the lighter color. Painting with white inverts the backdrop color;
        painting with black produces no change.}
    \showmode{exclusion} & \verb|exclusion| & \showmodeR{Produces an effect similar to that of the Difference
        mode but lower in contrast. Painting with white inverts the backdrop
        color; painting with black produces no change.}
    \showmode{hue} & \verb|hue| & \showmodeR{Creates a color with the hue of the source color and the
        saturation and luminosity of the backdrop color.}
    \showmode{saturation} & \verb|saturation| & \showmodeR{Creates a color with the saturation of the source
    ...
    \showmode{color} & \verb|color| & \showmodeR{Creates a color with the hue and saturation of the source
    ...
    \showmode{luminosity} & \verb|luminosity| & \showmodeR{Creates a color with the luminosity of the source
...
\begin{environment}{{tikzfadingfrompicture}\oarg{options}}
    ...
    %
\begin{codeexample}[preamble={\usetikzlibrary{fadings,patterns}},pre={\begin{lateximage}},post={\end{lateximage}},svg]
...
\end{codeexample}
    ...
    a shading.
    %
\begin{codeexample}[preamble={\usetikzlibrary{fadings,patterns}},svg]
\begin{tikzpicture}
  % Checker board
...
pleasing effects:
%
\begin{codeexample}[preamble={\usetikzlibrary{fadings,patterns,shadows}},pre={\begin{lateximage}},post={\end{lateximage}}]
...
\begin{key}{/tikz/transparency group=\oarg{options}}
    ...
    group on the page.
    %
\begin{codeexample}[preamble={\usetikzlibrary{patterns,shapes.symbols}},svg]
\begin{tikzpicture}
