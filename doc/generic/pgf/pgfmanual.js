...
function makeAnchorButtons() {
  ...
  // Check if both data-pdf-link and data-html-link attributes are set
  if (element.dataset.pdfLink && element.dataset.htmlLink) {
    ...
    // Replace the original element with the new PDF and HTML links
    element.replaceWith(pdfLink, htmlLink);
  } else if (element.dataset.htmlLink) {
    // Create the HTML link element
    const htmlLink = document.createElement('a');
    htmlLink.href = `#${element.dataset.htmlLink}`;
    htmlLink.className = 'anchor-html-link';
    ...
    // Replace the original element with the new HTML link
    element.replaceWith(htmlLink);
  }
});
}
