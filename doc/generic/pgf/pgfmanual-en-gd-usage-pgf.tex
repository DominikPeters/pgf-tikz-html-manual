...
\section[gd-usage-pgf]{Using Graph Drawing in PGF}
...
\begin{tikzpicture}[
    ...
    \node (tikz-begin-graph) at (0,3) [object node] {\texttt{\textbackslash{}graph[... layout]\{}; %}
    \node (tex-begin-graph) at (6,3) [object node] {\texttt{\textbackslash{}pgfgdbeginscope}};
    \node (interface-new-graph) at (13,3) [object node] {\texttt{beginGraphDrawingScope(...)}};
    ...
    % and one edge
    \node (tikz-node) at (0,2) [object node] {\texttt{a -> b;}};
    \node (tex-node) at (6,2) [object node, double copy shadow] {\texttt{\textbackslash{}pgfnode}};
    \draw[p] (tikz-node.east) -- (tex-node.west);

    \node (interface-add-node) at (13,2) [object node, double copy shadow] {\texttt{createVertex(...)}};
    \draw[p] (tex-node.east) -- (interface-add-node.west);

    \node (tex-add-edge) at (6,1) [object node, double copy shadow] {\texttt{\textbackslash{}pgfgdedge}};
    \node (interface-add-edge) at (13,1) [object node, double copy shadow] {\texttt{createEdge(...)}};
    ...
    \node (tikz-end) at (0,0) [object node] {\texttt{\};}};
    \node (tex-end) at (6,0) [object node] {\texttt{\textbackslash{}pgfgdendscope}};
    \node (interface-draw-graph) at (13,0) [object node] {\texttt{runGraphDrawingAlgorithm()}};
    \node (interface-finish-graph) at (13,-2) [object node] {\texttt{endGraphDrawingScope()}};
